...
def rateGuess(myGuess, word):
    ...
    for spot in range(5):
        ...
        if inSpot(myLetter, word, spot) == True:
            feedback = feedback + myLetter.upper() #correct letter in location of word
        elif inWord(myLetter, word) == True:
            feedback = feedback + myLetter.lower() #letter in word, wrong location in word
        else:
            feedback = feedback + "*"
...
def main():
    ...
    while guessNum <= 6:
        ...
        while validWord == False:
            ...
            guess = guess.lower()
            remain = 6 - guessNum
            attempts = str(remain) + " tries remain."
            if guess not in wordList:
                print("That word's not in the available list.")
                print(attempts)
                validWord = False
            else:
                validWord = True
